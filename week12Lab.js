/* 
  ...
  7/17/2024 Bob Ruzga Solutions for Week 12 Lab
*/
 ...
 * ↓ YOUR CODE HERE ↓ */
  ...
  const jsonSvrUrl = "http://localhost:3000/studentRoster";

  console.log('Using: ' + jsonSvrUrl);
 ...
 *
 * ↓ YOUR CODE HERE ↓ */
 // $.get(jsonSvrUrl).then(data => console.log(data));   //P2 S1
  
 $.get(jsonSvrUrl).then(data => {
    data.map(student => {
      $('tbody').append(
        $(`
          <tr>
            ...
            <td><button id="deleteRow" class="btn btn-outline-danger" onclick="deleteUser(${student.id})">🗑</button</td>
          </tr>
          `)
      )
    })
...
console.log('Part 2 Successfully completed! ------------');
...
$('#newStudentBtn').click(function () {
    let inStudentName = $('#aStudentName').val();
    let inReseachAssignment = $('#aResearchAssignment').val();
   
    let newData = JSON.stringify({ 
      "fullName": inStudentName,
      "researchAssignment": inReseachAssignment
    })
    
    $.post(jsonSvrUrl, newData)  //NB: Hit the unexpected token error similar to Week 11 Lab. Used Stringify to resolve here too!
  })

console.log('Part 3 COMPLETED! *** NB: ID is not sequential (mix of nums/letters) eventho I deleted db.json as per video ------')
...
// Even after deleting the db.json file contents, I still get the weird id's unlike solution video suggests.
// I can correct the id in the db.json file and things work as expected.
...
console.log('Part 4 Deleting Successfully Completed')
 ...
 * ↓ YOUR CODE HERE ↓ */
function updateUser(){
  let id = $('#uStudentId').val();
  let upStudentName = $('#uStudentName').val();
  let upReseachAssignment = $('#uResearchAssignment').val();
   
    let newData = JSON.stringify({ 
      "fullName": upStudentName,
      "researchAssignment": upReseachAssignment,
    }) 
...
$('#updStudentBtn').click(updateUser);

console.log ('-------- Bobs Project is Successfully Completed!! -----')
